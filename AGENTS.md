...
## Testing
- **CI**: `.github/workflows/ci.yml` runs `bin/install-local.sh` on every push to `main`, weekly (Mondays), and on manual dispatch.
- **Local Docker test**: `bin/test-container.sh` builds an Ubuntu container with SSH, generates a temp SSH key, and runs `remote.yml` against it. It auto-cleans the container and temp files on exit/error.
...
- **Environment**: Playbooks rely on the `USER` environment variable for user-specific tasks (e.g., adding to the `docker` group).
